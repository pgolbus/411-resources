...
add_boxer_errors() {
  ...
  echo "Testing invalid boxer input: $name"
  response=$(curl -s -X POST "$BASE_URL/add-boxer" -H "Content-Type: application/json" \
    -d "{\"name\": \"$name\", \"weight\": $weight, \"height\": $height, \"reach\": $reach, \"age\": $age}")
  ...
  if echo "$response" | grep -q '"status": "error"'; then
    ...
    echo "Error: Invalid boxer accepted: $name"
    echo "$response"
    exit 1
  fi
}
...
delete_boxer_by_id() {
  ...
  if echo "$response" | grep -q '"status": "success"'; then
    echo "Successfully deleted boxer ID $boxer_id"
  else
...
fight() {
  ...
  if echo "$response" | grep -q '"status": "success"'; then
    winner=$(echo "$response" | jq -r '.data.winner')
    echo "Fight completed. Winner: $winner"
  else
    echo "Fight failed."
    echo "$response"
    exit 1
