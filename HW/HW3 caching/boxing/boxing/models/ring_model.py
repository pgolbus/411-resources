...
class RingModel:
    ...
    def enter_ring(self, boxer_id: int):
        ...
        """
        logger.info(f"Received request for boxer with ID {boxer_id} to enter the ring")


        if boxer_id in self.playlist:
            logger.error(f"Boxer with ID {boxer_id} already exists in the ring")
            raise ValueError(f"Song with ID {boxer_id} already exists in the playlist")
