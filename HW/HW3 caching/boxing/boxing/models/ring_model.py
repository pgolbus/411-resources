...
class RingModel:
    ...
    def __init__(self):
        ...
        self.ttl_seconds = int(os.getenv("TTL", 60))  # Default 60 seconds TTL
    ...
    def enter_ring(self, boxer_id: int):
        ...
        self.ring.append(boxer_id)  # ADD boxer to the ring
        self._boxer_cache[boxer_id] = boxer  # Cache boxer
        self._ttl[boxer_id] = time.time() + self.ttl_seconds  # Set TTL

        logger.info(f"Adding boxer '{boxer.name}' (ID {boxer_id}) to the ring")
         
        current_boxers = [Boxers.get_boxer_by_id(b).name for b in self.ring]
