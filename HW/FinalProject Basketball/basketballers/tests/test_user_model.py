import pytest
from basketballers.models.user_model import User
...
def test_check_password_correct(session, sample_user):
     ...
     User.create_user(**sample_user)
     assert User.check_password(sample_user["username"], sample_user["password"]) is True

def test_check_password_incorrect(session, sample_user):
     """Test checking an incorrect password."""
     User.create_user(**sample_user)
     assert User.check_password(sample_user["username"], "wrongpassword") is False
...
def test_update_password(session, sample_user):
     ...
     assert User.check_password(sample_user["username"], new_password) is True
...
def test_get_id_by_username(session, sample_user):
     ...
     User.create_user(**sample_user)
     user_id = User.get_id_by_username(sample_user["username"])
     user = session.query(User).filter_by(username=sample_user["username"]).first()
...
def test_get_id_by_username_user_not_found(session):
     ...
     with pytest.raises(ValueError, match="User nonexistentuser not found"):
         User.get_id_by_username("nonexistentuser")
